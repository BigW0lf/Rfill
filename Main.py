...
from process_surf import (
    ...
    TCD2Tab, Tab_output, export_tables_to_excel, update_glossary,
)
...
class SurfaceApp(tk.Tk):
    ...
    def update_glossaire_tab(self):
        ...
        for df_t in self.df_types:
            ...
            if t not in self.super_cats:
                self.super_cats[t] = {sc: [] for sc in self._DEFAULT_SUPER_CATS}
                self.super_cats[t][self._DEFAULT_SUPER_CATS[0]] = list(known_cats)
            else:
                already = {c for cats in self.super_cats[t].values() for c in cats}
                new_cats = [c for c in known_cats if c not in already]
                if new_cats:
    ...
    def load_glossaire_board(self, event=None):
        ...
        for sc_name, sc_cats in sc_dict.items():
            ...
            tk.Label(sc_hdr, text=sc_name, font=("Arial", 11, "bold"),
                     bg=self._SC_HDR_BG).pack(side="left", padx=10)
            ...
            for cat in all_sc_cats:
                ...
                lbl = tk.Label(hdr, text=cat, font=("Arial", 10, "bold"),
                               bg=color, wraplength=120, justify="center", cursor="hand2")
    ...
    def build_generer_tab(self):
        ...
        ttk.Label(frame, text="Export Excel",
                  style="Title.TLabel").pack(anchor="w", pady=(0, 8))
        ttk.Label(frame,
                  text="Un onglet par type de surface sera créé dans le fichier.",
        ...
        ttk.Button(
            action_row, text="  Générer et sauvegarder le fichier Excel  ",
    ...
    def generate_final_tables(self):
        ...
        try:
            ...
            self._log(self.text_generer, f"[ERR]  Export → {e}", "err")
    ...
    def build_aide_tab(self):
        ...
        section("3 · Générer",
                "Renseignez les informations du projet (bâtiment, adresse, propriétaire, cadastre…) "
                "puis cliquez sur « Générer et sauvegarder le fichier Excel ».\n\n"
                "• Un onglet Excel est créé par type de surface.\n"
                "• Chaque onglet contient un en-tête projet, un tableau pivotant par étage/occupant/catégorie, "
                "des sous-totaux par sur-catégorie et un total général.\n"
                "• Les lignes « Total étage » apparaissent en bleu clair ; le « TOTAL » général en bleu foncé.")
